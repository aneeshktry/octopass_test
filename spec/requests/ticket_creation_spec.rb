...
RSpec.describe "Create ticket", :type => :request do
  ...
  it "creates a ticket and return the response" do
    headers = { "CONTENT_TYPE" => "application/json" }
    post "/api/v1/tickets", :params => JSON_DATA.to_json , :headers =>headers
    json_response = JSON.parse(response.body)
    expect(json_response.keys).to match_array(["status", "message"])
    expect(response.body).to include("Successfully created ticket and excavator") 
  end

  it "should return a reponse with error messages" do
    headers = { "CONTENT_TYPE" => "application/json" }
